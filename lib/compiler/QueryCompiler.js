...
class QueryCompiler {
  ...
  QueryNode(node) {
    ...
    if (node.from.length !== 0) {
      ...
      if ((node.queryType === 'select' || node.queryType === 'update') && node.fromOptions.only) {
        sql += 'only ';
      }
  ...
  AliasableNode(node) {
    ...
    sql += node.identifier.visit(this);
    ...
    sql += rows
      .map(row => {
        ...
        keys.forEach(key => {
          ...
          values.push(value ? value : this.opt.insertDefaultValue);
        });
  ...
  FilterNode(node, { idx }) {
    ...
    // knex quirk: where in with empty list is rendered as 1 = 0.
    if (op && op.includes('in') && node.rhs.type === 'ListNode' && node.rhs.items.length === 0) {
      if (sql) sql += ' ';

      sql += '1 = ' + this.addBinding(node.not ? 1 : 0);
      return sql;
    }
    ...
    if (node.rhs) {
      ...
      if (op.includes('between')) {
        sql += node.rhs.items.map((it, idx) => it.visit(this, { idx })).join(' and ');
      } else if (op.includes('in') && node.rhs.type === 'ListNode') {
        sql += '(' + node.rhs.items.map((it, idx) => it.visit(this, { idx })).join(', ') + ')';
      } else {
  ...
  IdentifierNode(node) {
    ...
    if (parent && (parent.type === 'FromNode' || parent.type === 'JoinNode')) {
      const schema = this.parents.get(parent).schema;

      if (schema) {
        wrappedIds.unshift(schema.visit(this));
      }
    }
  ...
  doWrapIdentifier(id) {
    ...
    if (this.opt.wrapIdentifier) {
      return this.opt.wrapIdentifier(id, id => this.wrapIdentifier(id));
    } else {
      return this.wrapIdentifier(id);
    }
  }
