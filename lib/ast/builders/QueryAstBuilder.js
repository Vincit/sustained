...
class QueryAstBuilder extends AstBuilder {
  ...
  insert(insert, returning) {
    ...
    this.ast.insert = parseInsert([insert], {
      queryBuilderClass: this.constructor,
      subQueryBuilderClass: this.constructor
    });
  ...
  update(...args) {
    ...
    this.ast.update = parseUpdate(args, {
      queryBuilderClass: this.constructor,
      subQueryBuilderClass: this.constructor
    });
  ...
  limit(...args) {
    this.ast.limit = this.runHooks(parseValue(...args));
    return this;
  }

  offset(...args) {
    this.ast.offset = this.runHooks(parseValue(...args));
    return this;
  }
  ...
  selectFunction(funcName, funcModifier, args) {
    ...
    nodes.forEach(node => {
      if (node instanceof SelectNode) {
        node.identifier = FunctionNode.create(funcName, [funcModifier], [node.identifier]);
      }
    });
