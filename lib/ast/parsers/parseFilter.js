...
const { isAstNode, isAstBuilder, isPlainObject, isFunction, isBoolean } = require('../../utils/typeUtils');
...
function parseThreeArg(args, opts) {
  if (args[0] === null) {
    return [
      opts.NodeClass.create(null, parseOperator(args[1]), parseValue(args[2], opts), opts.bool, opts.not)
    ];
  } else {
    return [
      opts.NodeClass.create(
