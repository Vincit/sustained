import { QueryBuilder, raw } from '../../../src'
import { expect } from '../../testUtils'
import { PostgreSQLQueryCompiler } from '../../../src/dialect/postgresql/PostgreSQLQueryCompiler'
...
describe('QueryBuilder', function () {
  ...
  it('and where betweens', function () {
    testsql(
      qb().select('*').from('users').where('name', '=', 'user1').andWhereBetween('id', [1, 2]),
      {
        mysql: {
  ...
  it('and where not betweens', function () {
    testsql(
      qb().select('*').from('users').where('name', '=', 'user1').andWhereNotBetween('id', [1, 2]),
      {
        mysql: {
  ...
  it('raw or havings', function () {
    testsql(
      qb().select('*').from('users').having('baz', '=', 1).orHaving(raw('user_foo < user_bar')),
      {
  ...
  it('or having between', function () {
    testsql(
      qb().select('*').from('users').havingBetween('baz', [5, 10]).orHavingBetween('baz', [20, 30]),
      {
  ...
  it('where exists with builder', function () {
    testsql(
      qb()
        .select('*')
        .from('orders')
        .whereExists(qb().select('*').from('products').whereRaw('products.id = orders.id')),
      {
        mysql: {
          sql:
            'select * from `orders` where exists (select * from `products` where products.id = orders.id)',
  ...
  it('full outer join', function () {
    testsql(
      qb().select('*').from('users').fullOuterJoin('contacts', 'users.id', '=', 'contacts.id'),
      {
        mssql: {
  ...
  it('cross join on', function () {
    testsql(
      qb().select('*').from('users').crossJoin('contracts', 'users.contractId', 'contracts.id'),
      {
        mysql: {
          sql:
  ...
  it('update only method', function () {
    testsql(
      qb().update({ email: 'foo', name: 'bar' }).table('users', { only: true }).where('id', '=', 1),
      {
        postgres: {
  ...
  it('update method with limit mysql', function () {
    ...
    testsql(
      qb().from('users').where('users.id', '=', 1).update({ email: 'foo', name: 'bar' }).limit(1),
      {
        mysql: {
  ...
  it('#287 - wraps correctly for arrays', function () {
    ...
    testsql(
      qb().select('*').from('value').join('table', 'table.array_column[1]', '=', raw('?', 1)),
      {
        mysql: {
    ...
    testsql(
      qb()
        .select(
          'e.lastname',
          'e.salary',
          raw(qb().select('avg(salary)').from('employee').whereRaw('dept_no = e.dept_no')).wrap(
            '(',
            ') avg_sal_dept'
          )
        )
  ...
  it('allows a raw query in the second param', function () {
    testsql(
      qb()
        ...
        .innerJoin(
          'table1',
          raw('ST_Contains(buildings_pluto.geom, ST_Centroid(buildings_building.geom))')
        ),
      {
        mysql: {
          sql:
  ...
  it('allows for raw values in join, #441', function () {
    testsql(
      qb()
        ...
        .innerJoin(
          raw(['SELECT MIN(nid) AS location_id', 'FROM nidmap2'].join(' ')).wrap('(', ') AS B'),
          'A.x',
          '=',
          'B.x'
        ),
  ...
  it.skip('#1228 Named bindings', function () {
    ...
    var mssqlQb = mssql
      ...
      .select('*')
      .from('users')
      .where(mssql.raw(':name: = :thisGuy or :name: = :otherGuy', namedBindings))
      .toSQL()
    var mysqlQb = mysql
      .queryBuilder()
      .select('*')
      .from('users')
      .where(mysql.raw(':name: = :thisGuy or :name: = :otherGuy', namedBindings))
      .toSQL()
    var sqliteQb = sqlite3
      .queryBuilder()
      .select('*')
      .from('users')
      .where(sqlite3.raw(':name: = :thisGuy or :name: = :otherGuy', namedBindings))
      .toSQL()
    ...
    testsql(
      qb()
        .with('withClause', function () {
          this.with(
            'withSubClause',
            raw('select "foo" as "baz" from "users" where "baz" > ? and "baz" < ?', [1, 20])
          )
            .select('*')
            .from('withSubClause')
        })
  ...
  describe('#2263, update / delete queries in with syntax', () => {
    it('with update query passed as raw', () => {
      testquery(
        qb()
          .with('update1', raw('??', [qb().from('accounts').update({ name: 'foo' })]))
          .from('accounts'),
        {
    ...
    it('with update query passed as callback', () => {
      testquery(
        qb()
          .with('update1', (builder) => builder.from('accounts').update({ name: 'foo' }))
          .from('accounts'),
        {
          postgres: `with "update1" as (update "accounts" set "name" = 'foo') select * from "accounts"`,
        }
    ...
    it('with delete query passed as raw', () => {
      testquery(
        qb()
          .with('delete1', raw('??', [qb().delete().from('accounts').where('id', 1)]))
          .from('accounts'),
        {
    ...
    it('with delete query passed as callback', () => {
      testquery(
        qb().with('delete1', qb().delete().from('accounts').where('id', 1)).from('accounts'),
        {
          postgres: `with "delete1" as (delete from "accounts" where "id" = 1) select * from "accounts"`,
        }
      )
